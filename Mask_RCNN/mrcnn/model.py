...
class DataGenerator(keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        return self.__data_generation(self.image_ids[index*self.batch_size:(index+1)*self.batch_size])
    ...
    def __data_generation(self, image_id_list):
        'Generates data containing batch_size samples'
        blist = []
        b, error_count = 0, 0
        try:
          blist = self.first_image_gen(image_id_list[0])
          for b in range(1, len(image_id_list)):
            self.data_gen(b, image_id_list[b], blist)
        except (GeneratorExit, KeyboardInterrupt):
            raise
        except:
            # Log it and skip the image
            logging.exception("Error processing image {}".format(
                self.dataset.image_info[image_id_list[b]]))
            error_count += 1
            if error_count > 5:
                raise
        ...
        if self.random_rois:
          if self.detection_targets:
              batch_images, batch_image_meta, batch_rpn_match, batch_rpn_bbox, \
                batch_gt_class_ids, batch_gt_boxes, batch_gt_masks, batch_rpn_rois, \
                batch_rois, batch_mrcnn_class_ids, batch_mrcnn_bbox, batch_mrcnn_mask = inputs
              # Keras requires that output and targets have the same number of dimensions
              batch_mrcnn_class_ids = np.expand_dims(
                  batch_mrcnn_class_ids, -1)
              outputs.extend(
    ...
    def first_image_gen(self, image_id):
      # If the image source is not to be augmented pass None as augmentation
      ...
      # have any of the classes we care about.
      if not np.any(gt_class_ids > 0):
        raise ValueError("Data generation error; image id: " + image_id + " has no instance")
      ...
      batch_images[0] = mold_image(image.astype(np.float32), self.config)
      ...
      if self.random_rois:
          batch_rpn_rois[0] = rpn_rois
          if self.detection_targets:
              batch_rois[0] = rois
              batch_mrcnn_class_ids[0] = mrcnn_class_ids
              batch_mrcnn_bbox[0] = mrcnn_bbox
              batch_mrcnn_mask[0] = mrcnn_mask

      blist = [batch_images, batch_image_meta, batch_rpn_match, batch_rpn_bbox,
        batch_gt_class_ids, batch_gt_boxes, batch_gt_masks]
      if self.random_rois:
        random_list = [batch_rpn_rois[b]]
        if self.detection_targets:
          random_list.extend([batch_rois, batch_mrcnn_class_ids, batch_mrcnn_bbox, batch_mrcnn_mask])
        blist.extend(random_list)
      return blist


    def data_gen(self, b, image_id, blist):
      if self.random_rois:
        if self.detection_targets:
          batch_images, batch_image_meta, batch_rpn_match, batch_rpn_bbox, \
            batch_gt_class_ids, batch_gt_boxes, batch_gt_masks, batch_rpn_rois, \
            batch_rois, batch_mrcnn_class_ids, batch_mrcnn_bbox, batch_mrcnn_mask = blist
        else:
          batch_images, batch_image_meta, batch_rpn_match, batch_rpn_bbox, \
            batch_gt_class_ids, batch_gt_boxes, batch_gt_masks,batch_rpn_rois  = blist
      else:
        batch_images, batch_image_meta, batch_rpn_match, batch_rpn_bbox, \
          batch_gt_class_ids, batch_gt_boxes, batch_gt_masks = blist
